...
const AddExpensesToGroup = () => {
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      api
        .get(`/spending?searchTitle=${searchTerm}`)
        .then(({ data }) => {
          setFilteredExpenses(data.content);
        });
    }, 500);
  ...
  const handleConfirmParticipants = () => {
    ...
    const selectedNamesArray = allExpenses
      .filter(expense => selectedexpenseIds.has(expense.id))
      .map(expense => expense.firstName);
  ...
  return (
    <div className="min-h-screen bg-white px-1 py-4 overflow-y-auto pb-24">
      ...
      <SubmitButton 
        ...
        text={`Confirmar Participantes (${selectedexpenseIds.size})`}
      />
    </div>
  );
};
